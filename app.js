...
class NomiApp {
    ...
    createContext(type) {
        ...
        // Build the object based on type
        const newObj = {
            id: id,
            ...
            gallery: [],
            custom_sections: [],
            // Groups start with just 'backstory', Nomis get the full default list
            sectionOrder: isGroup ? ['backstory'] : DEFAULT_SECTIONS.map(s => s.id),
            // Add unique fields: Groups get 'members', Nomis get 'data' & 'isFavorite'
    ...
    editContext(id, type, pushHistory = true, initialTab = 'profile') {
        ...
        // Ensure Section Order Exists (Legacy Fix)
    ...
    toggleBatchMode() {
        ...
        // NEW: Uses our helper to determine which buttons to toggle
    ...
    setContextPhoto(b64, optionalId) {
        // Handle lookup for any ID (Nomi or Group)
        ...
        if (target) {
            ...
            // Update UI if it's the currently viewed item
    ...
    addSection() {
        ...
        // Unique ID prefix based on type
